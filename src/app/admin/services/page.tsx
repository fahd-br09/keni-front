...
const empty = { titleAr: '', descriptionAr: '', category: '', requiredDocuments: [] as { nameAr: string }[] };
...
export default function AdminServicesPage() {
  ...
  const openEdit = (s: Service) => { setEditing(s); setForm({ titleAr: s.titleAr, descriptionAr: s.descriptionAr, category: s.category, requiredDocuments: s.requiredDocuments }); setModal(true); };
  ...
  const handleDelete = async (id: string) => {
    if (confirm('هل أنت متأكد من الحذف؟')) { await deleteService(id); load(); }
  };

  const addDoc = () => {
    if (docInput.trim()) { setForm(p => ({ ...p, requiredDocuments: [...p.requiredDocuments, { nameAr: docInput.trim() }] })); setDocInput(''); }
  };
            ...
            {services.map(s => (
              <tr key={s._id} className="hover:bg-gray-50 transition-colors">
                <td className="px-4 py-3">
                  ...
                  <div className="text-gray-400 text-xs line-clamp-1">{s.descriptionAr}</div>
                </td>
                <td className="px-4 py-3 hidden md:table-cell text-gray-500">{s.category || '—'}</td>
                <td className="px-4 py-3">
        ...
        <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl w-full max-w-lg shadow-xl">
            ...
            <div className="p-5 space-y-4 max-h-[70vh] overflow-y-auto">
              <input placeholder="اسم الخدمة *" value={form.titleAr} onChange={e => setForm(p => ({ ...p, titleAr: e.target.value }))}
                className="w-full border border-gray-200 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-green-500" />
              <input placeholder="التصنيف" value={form.category} onChange={e => setForm(p => ({ ...p, category: e.target.value }))}
                className="w-full border border-gray-200 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-green-500" />
              <textarea placeholder="الوصف *" rows={3} value={form.descriptionAr} onChange={e => setForm(p => ({ ...p, descriptionAr: e.target.value }))}
              ...
              <div>
                ...
                <div className="flex gap-2 mb-2">
                  <input placeholder="أضف وثيقة..." value={docInput} onChange={e => setDocInput(e.target.value)}
